...
df['date_numeric'] = df['Date'].astype('int64') // 10**9  # Convert nanoseconds to seconds - delt på 1000 igjen
...
def custom_loss(y_true, y_pred):
    ...
    k_pos = tf.nn.softplus(model.k)  # Sørger for at k > 0
    ...
    return -tf.reduce_mean(log_likelihood)  # Bruk gjennomsnitt i stedet for sum
...
# New train data set for aft model
...
# New test data set for aft model
...
# Calc CDF-value for each obs in test set
for i in range(len(X_test)):
    cdf_data = aft_model_nll_loss.predict_cumulative_hazard(df_combined_test.iloc[[i]])
